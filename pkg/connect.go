...
// ConnectConfig describes how to connect to gid-service. Mode selects the
...
// resource injection for module mode.
...
// Connect resolves a gid-service dependency end to end and registers its
...
// composition can share this instance downstream via WithGIDHandler.
func Connect(cfg ConnectConfig, mgr *lifecycle.Manager) (Service, *Handler, error) {
	switch cfg.Mode {
	case configx.ModeGRPC:
		if cfg.Target == "" {
			return nil, nil, fmt.Errorf("gid-service: target required when mode=grpc")
		}
		c, err := NewClient(cfg.Target)
		if err != nil {
			return nil, nil, fmt.Errorf("gid-service: %w", err)
		}
		mgr.AddStopper("gid-service", lifecycle.StopFunc(func() { _ = c.Close() }))
		return c, nil, nil
	case configx.ModeModule, configx.ModeUnspecified:
		if cfg.Config == nil {
			return nil, nil, fmt.Errorf("gid-service: module config required")
		}
		...
		return hdl, hdl, nil
	default:
		return nil, nil, fmt.Errorf("gid-service: unknown mode %q (want \"grpc\" or \"module\")", cfg.Mode)
	}
}
